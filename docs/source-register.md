...
| **SRC-020** | SPDX/Linux Foundation, [SPDX](https://spdx.dev/), [SPDX Specification v3.0.1](https://spdx.dev/wp-content/uploads/sites/31/2024/12/SPDX-3.0.1-1.pdf), [SPDX Specification v2.3](https://spdx.github.io/spdx-spec/v2.3/), and the [v2.3 JSON schema](https://github.com/spdx/spdx-spec/blob/v2.3/schemas/spdx-schema.json) | **SPDX 3.0.1**, stable current specification; **SPDX 2.3** retained as the declared GitHub SBOM-attestation exchange format, signed tag `v2.3`, commit `aadf3b0`; SPDX 3.1 release-candidate material is not adopted | **Normative — conditional syntax.** The version an artifact declares is binding for its SPDX conformance claim. The release builder emits SPDX 2.3 JSON; other 0.1.0-draft uses may explicitly select 3.0.1. Otherwise SPDX is an authoritative SBOM option. | Dependency inventory, SBOM identity, artifact/release metadata | 2026-08-09 | Quarterly while 3.1 matures, then on stable release; next 2026-11-09 |
